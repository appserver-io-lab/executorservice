...
namespace AppserverIo\Lab;

/**
 * Class ExecutorService
 *
 * This is a proof of concept how to handle and make use of an executor service
 * that can handle real singleton objects globally over thread contexts.
 *
 * @author    Johann Zelger <jz@appserver.io>
 * @copyright 2015 TechDivision GmbH <info@appserver.io>
 * @license   http://opensource.org/licenses/osl-3.0.php Open Software License (OSL 3.0)
...
class ExecutorService extends \Thread
{
     ...
     * @return string
     */
    static public function __getEntityKey($key)
    {
     ...
     * @return void
     */
    static public function __init($mainEntity = '\stdClass')
    {
     ...
     */
    static public function __getEntity($entityTypeOrAlias)
    {
        // get own instance
     ...
     * @throws \Exception
     */
    static public function __newFromEntity($entityType, $alias = null) {
        // get own instance
        $self = self::__getInstance();
        // init entity key
    ...
    public function __callback(callable $callback)
    {
        if ($this->__callbackAllowed === true) {
            $this->synchronized(function ($self, $callback) {
    ...
    public function __call($name, $args)
    {
        ...
        if (isset($this->methodExecutionTypeMapper["::{$name}"])) {
